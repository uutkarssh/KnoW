...
async function fetchWeather(message) {
  ...
  // Try Open-Meteo (free, no key, open CORS)
  try {
      ...
      if (geoData.results && geoData.results.length) {
        ...
        if (wRes.ok) {
          ...
          return {
            ...
            condition: codes[cur.weather_code] || 'Variable',
  ...
  // Fallback: Ask Groq for weather info
  return { city: city, source: 'groq' };
}
  ...
  try {
    if (intent === 'weather') {
      ...
      if (data && data.source === 'live') {
        responseText = renderWeather(data);
      } else if (data && data.source === 'groq') {
        // Fallback to Groq for weather info
        var weatherMsgs = [
          { role: 'system', content: 'You are a helpful weather assistant. Give current typical weather conditions, temperature ranges, humidity, and climate info for the given city. Be specific and informative. Format clearly with bold labels.' },
          { role: 'user', content: 'What is the weather like in ' + data.city + ' right now? Give temperature range, conditions, humidity.' }
        ];
        responseText = await callGroq(weatherMsgs);
      } else {
        responseText = 'Could not get weather for that location. Try: "weather in Mumbai"';
      }
